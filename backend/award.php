...
if (!$Auth->IsAdminLoggedIn()) {
    ...
    switch ($mode) {
        case 'export':
            ...
            $sql .= 'LEFT JOIN vote_log ON award.keychain=vote_log.keychain ORDER BY award_id ASC';
        ...
        default:
            ...
            $orderby = in_array($getOrderby, $orderbys) ? $getOrderby : 'award_id';
            ...
            if ($count_records > 0) {
                ...
                $sql = 'SELECT * FROM `award`';
                $sql .= 'LEFT JOIN vote_log ON award.keychain=vote_log.keychain';
                if (!empty($wheres)) {
                    $sql .= ' WHERE '.implode(' AND ', $wheres);
                }
            ...
            $tpl_file = ADMIN_PATH_TPL.'/award.html.php';
            break;
    }
}
